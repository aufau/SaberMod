...
void G_ReflectMissile( gentity_t *ent, gentity_t *missile, const vec3_t forward )
{
	...
	*/
	...
	if ( missile->r.ownerNum != ENTITYNUM_NONE && g_entities[missile->r.ownerNum].r.linked &&
		 missile->s.weapon != WP_SABER && missile->s.weapon != G2_MODEL_PART && !isowner )
...
void G_ExplodeMissile( gentity_t *ent ) {
	...
	if ( ent->splashDamage ) {
				...
				ent->splashMethodOfDeath ) )
		{
			if (ent->parent)
			{
				g_entities[ent->parent->s.number].client->pers.accuracy_hits++;
			}
			else if (ent->activator)
			{
				g_entities[ent->activator->s.number].client->pers.accuracy_hits++;
			}
		}
	}
...
void G_RunMissile( gentity_t *ent ) {
	...
	G_Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, ent->clipmask );

	if ( tr.startsolid || tr.allsolid ) {
		// make sure the tr.entityNum is set to the entity we're stuck in
		G_Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, passent, ent->clipmask );
		tr.fraction = 0;
	}
	else {
	...
	if (ent->s.weapon == G2_MODEL_PART && !ent->bounceCount)
	{
		...
		lowerOrg[2] -= 1;
		G_Trace( &trG, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, lowerOrg, passent, ent->clipmask );
