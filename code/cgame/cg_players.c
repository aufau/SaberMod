...
static void CG_RefereeMode(qboolean enable) {
	...
	UpdateCommand("ref_referee");
	UpdateCommand("ref_unreferee");
	UpdateCommand("ref_lockteam");
	UpdateCommand("ref_unlockteam");
	UpdateCommand("ref_forceteam");
	UpdateCommand("ref_announce");
}
...
//	byte			colors[4];
	...
	numFragments = trap_CM_MarkFragments( 4, (const float (*)[3])originalPoints,
					projection, MAX_MARK_POINTS, markPoints, MAX_MARK_FRAGMENTS, markFragments );
...
doEssentialThree:
	...
	if (cg_entities[cent->currentState.number].teamPowerEffectTime > cg.time && cg_entities[cent->currentState.number].teamPowerType == TFP_ABSORB)
