...
int OrgVisible(vec3_t org1, vec3_t org2, int ignore)
{
	...
	G_Trace(&tr, org1, NULL, NULL, org2, ignore, MASK_SOLID);
...
int WPOrgVisible(gentity_t *bot, vec3_t org1, vec3_t org2, int ignore)
{
	...
	if (tr.fraction == 1)
	{
		G_Trace(&tr, org1, NULL, NULL, org2, ignore, MASK_PLAYERSOLID);
...
int OrgVisibleBox(vec3_t org1, vec3_t mins, vec3_t maxs, vec3_t org2, int ignore)
{
	...
	G_Trace(&tr, org1, mins, maxs, org2, ignore, MASK_SOLID);
...
int CheckForFunc(vec3_t org, int ignore)
{
	...
	G_Trace(&tr, org, NULL, NULL, under, ignore, MASK_SOLID);
...
static int BotTrace_Strafe(bot_state_t *bs, const vec3_t traceto)
{
	...
	G_Trace(&tr, from, playerMins, playerMaxs, to, bs->client, MASK_PLAYERSOLID);
...
static int BotTrace_Jump(bot_state_t *bs, const vec3_t traceto)
{
	...
	G_Trace(&tr, bs->origin, mins, maxs, traceto_mod, bs->client, MASK_PLAYERSOLID);
	...
	G_Trace(&tr, tracefrom_mod, mins, maxs, traceto_mod, bs->client, MASK_PLAYERSOLID);
	...
	for (i = 0; i < num; i++)
	{
		...
		{ //try to escape from anything with a non-0 s.weapon and non-0 damage. This hopefully only means dangerous projectiles.
		  //Or a sentry gun if bolt_Head == 1000. This is a terrible hack, yes.
			...
			{
				G_Trace(&tr, bs->origin, NULL, NULL, ent->s.pos.trBase, bs->client, MASK_SOLID);
...
{ //get the nearest possible waypoint to the flag since it's not in its original position
	...
	{
		G_Trace(&tr, wp->origin, mins, maxs, flagEnt->s.pos.trBase, flagEnt->s.number, MASK_SOLID);
	...
	while (i < gWPNum)
	{
		...
		{
			G_Trace(&tr, gWPArray[i]->origin, mins, maxs, flagEnt->s.pos.trBase, flagEnt->s.number, MASK_SOLID);
...
success:
	...
	(void)weHaveEnemyFlag; // suppress warning
...
int Saga_TargetClosestObjective(bot_state_t *bs, int flag)
{
	...
	if ( bs->wpDestination && (bs->wpDestination->flags & flag) && bs->wpDestination->associated_entity != ENTITYNUM_NONE &&
		 bs->wpDestination->associated_entity != ENTITYNUM_NONE && g_entities[bs->wpDestination->associated_entity].use )
	{
		goto hasPoint;
	}
	...
	if (bs->sagaState == SAGASTATE_ATTACKER)
	{
		...
		{ //looks like we have no goals other than to keep the other team from completing objectives
			...
			if (bs->shootGoal)
			{
				...
				{
					G_Trace(&tr, bs->origin, NULL, NULL, dif, bs->client, MASK_SOLID);
...
void MeleeCombatHandling(bot_state_t *bs)
{
	...
	G_Trace(&tr, usethisvec, mins, maxs, downvec, ENTITYNUM_NONE, MASK_SOLID);
	...
	G_Trace(&tr, bs->origin, mins, maxs, downvec, ENTITYNUM_NONE, MASK_SOLID);
	...
	G_Trace(&tr, midorg, mins, maxs, downvec, ENTITYNUM_NONE, MASK_SOLID);
...
void SaberCombatHandling(bot_state_t *bs)
{
		...
		en_down == mid_down)
	{
		...
		else if (bs->currentEnemy->client->ps.weapon == WP_SABER && bs->frame_Enemy_Len < 80 && ((Q_irand(1, 10) < 8 && bs->saberBFTime < level.time) || bs->saberBTime > level.time))
		{
			...
			G_Trace(&tr, bs->goalPosition, NULL, NULL, groundcheck, bs->client, MASK_SOLID);
...
int BotFallbackNavigation(bot_state_t *bs)
{
	...
	G_Trace(&tr, bs->origin, mins, maxs, trto, ENTITYNUM_NONE, MASK_SOLID);
...
void StrafeTracing(bot_state_t *bs)
{
	...
	G_Trace(&tr, bs->origin, mins, maxs, rorg, bs->client, MASK_SOLID);
	...
	G_Trace(&tr, rorg, NULL, NULL, drorg, bs->client, MASK_SOLID);
...
gentity_t *CheckForFriendInLOF(bot_state_t *bs)
{
	...
	G_Trace(&tr, trfrom, mins, maxs, trto, bs->client, MASK_PLAYERSOLID);
	...
	if (flagRed && flagBlue)
	{
			...
			bs->wpDestination == flagBlue)
		{
			...
			if (diddrop && desiredDrop)
			{
				...
				if (VectorLength(a) <= BOT_FLAG_GET_DISTANCE)
				{
					G_Trace(&tr, bs->origin, mins, maxs, desiredDrop->s.pos.trBase, bs->client, MASK_SOLID);
...
int BotSurfaceNear(bot_state_t *bs)
{
	...
	G_Trace(&tr, bs->origin, NULL, NULL, fwd, bs->client, MASK_SOLID);
