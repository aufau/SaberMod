...
static void UI_DrawHandicap(rectDef_t *rect, float scale, vec4_t color, int textStyle, font_t iMenuFont) {
  ...
  h = trap_Cvar_VariableValue("handicap");
  h = Com_Clamp( 5, 100, h );
  i = 20 - h / 5;
	...
	if (uiInfo.serverStatus.motdLen) {
		...
		if (uiInfo.uiDC.realTime > uiInfo.serverStatus.motdTime) {
			...
			if (uiInfo.serverStatus.motdPaintX <= rect->x + 2) {
				if (uiInfo.serverStatus.motdOffset < uiInfo.serverStatus.motdLen) {
					uiInfo.serverStatus.motdPaintX += Text_Width(&uiInfo.serverStatus.motd[uiInfo.serverStatus.motdOffset], scale, iMenuFont) - 1;
...
static qboolean UI_OwnerDrawVisible(int flags) {
	...
	while (flags) {
		...
		if (flags & UI_SHOW_ANYTEAMGAME) {
			if (!GT_Team(uiInfo.gameTypes[ui_gameType.integer].gtEnum)) {
				vis = qfalse;
			}
		...
		if (flags & UI_SHOW_NETANYTEAMGAME) {
			if (!GT_Team(uiInfo.gameTypes[ui_netGameType.integer].gtEnum)) {
				vis = qfalse;
			}
...
static qboolean UI_Handicap_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		int h = trap_Cvar_VariableValue("handicap");
		if (key == A_MOUSE2) {
			h -= 5;
		} else {
			h += 5;
...
static qboolean UI_NetGameType_HandleKey(int flags, float *special, int key) {
  if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
	...
	trap_Cvar_Set( "ui_netGameType_fixed", va("%d", ui_netGameType.integer));
...
static qboolean UI_JoinGameType_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		trap_Cvar_Set( "ui_joinGameType_fixed", va("%d", ui_joinGameType.integer));
...
static qboolean UI_NetSource_HandleKey(int flags, float *special, int key) {
  if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
	...
	trap_Cvar_Set( "ui_netSource_fixed", va("%d", ui_netSource.integer));
    return qtrue;
  }
  return qfalse;
}
		...
		if (Q_stricmp(name, "StartServer") == 0)
		{
			...
			for (i = 0; i < PLAYERS_PER_TEAM; i++)
			{
				...
				if (bot > 1)
				{
					...
					if (numval <= maxcl)
					{
						if (GT_Team(ui_actualNetGameType.integer)) {
...
void _UI_Init( qboolean inGameLoad ) {
  ...
  //UI_Load();
	...
	trap_SP_Register("sabermod2_menus");
...
static cvarTable_t cvarTable[] = {
	...
	{ &ui_joinGameType, "ui_joinGametype_fixed", "0", CVAR_ARCHIVE },
	{ &ui_netGameType, "ui_netGametype_fixed", "0", CVAR_ARCHIVE },
	...
	{ &ui_netSource, "ui_netSource_fixed", "0", CVAR_ARCHIVE },
