...
void FX_DisruptorAltMiss( vec3_t origin, vec3_t normal )
{
	...
	*/
	...
	b.rgbParm = 0.0f;
	b.killTime = 4000;
	b.shader = cgs.media.smokeTrail;
	b.flags = FX_ALPHA_WAVE;

	trap_FX_AddBezier(&b);

	trap_FX_PlayEffectID( cgs.effects.disruptorAltMissEffect, origin, normal );
}
...
{
	trap_FX_PlayEffectID( cgs.effects.disruptorAltHitEffect, origin, normal );
}
...
{
	trap_FX_PlayEffectID( cgs.effects.disruptorWallImpactEffect, origin, normal );
}
...
{
	trap_FX_PlayEffectID( cgs.effects.disruptorFleshImpactEffect, origin, normal );
}
