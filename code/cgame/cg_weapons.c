...
void CG_AddViewWeapon( const playerState_t *ps ) {
	...
	if ( !cg_drawGun.integer || cg.predictedPlayerState.zoomMode != ZOOM_NONE) {
		vec3_t		origin;
...
void CG_CheckPlayerG2Weapons(const playerState_t *ps, centity_t *cent)
{
		...
		ps->clientNum == cent->currentState.number) //don't want spectator mode forcing one client's weapon instance over another's
	{
		...
		if (cent->weapon == WP_SABER && (weapon_t)cg_entities[cent->currentState.number].weapon != ps->weapon && !ps->saberHolstered)
		...
		else if (ps->weapon == WP_SABER && (weapon_t)cg_entities[cent->currentState.number].weapon != ps->weapon && !ps->saberHolstered)
