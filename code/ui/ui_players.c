...
//	refEntity_t		barrel;
	...
	refdef.fov_x = 90.0f / 640 * refdef.width;
	xx = refdef.width / tanf( DEG2RAD( 0.5f * refdef.fov_x ) );
	refdef.fov_y = RAD2DEG( 2 * atan2f( refdef.height, xx ) );

	// calculate distance so the player nearly fills the box
	len = 0.7 * ( maxs[2] - mins[2] );
	origin[0] = len / tanf( DEG2RAD( 0.5f * refdef.fov_x ) );
...
*/
	...
	if ( dp_realtime <= pi->muzzleFlashTime ) {
		...
		if ( pi->flashDlightColor[0] || pi->flashDlightColor[1] || pi->flashDlightColor[2] ) {
			trap_R_AddLightToScene( flash.origin, 200 + (id_rand()&31), pi->flashDlightColor[0],
