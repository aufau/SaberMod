...
{
	trap_FX_PlayEffectID( cgs.effects.blasterWallImpactEffect, origin, normal );
}
...
void FX_BlasterWeaponHitPlayer( vec3_t origin, vec3_t normal, qboolean humanoid )
{
	if ( humanoid )
	{
		trap_FX_PlayEffectID( cgs.effects.blasterFleshImpactEffect, origin, normal );
	}
	else
	{
		trap_FX_PlayEffectID( cgs.effects.blasterDroidImpactEffect, origin, normal );
	}
}
