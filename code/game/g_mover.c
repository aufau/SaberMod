...
gentity_t	*G_TestEntityPosition( gentity_t *ent ) {
	...
	if ( ent->client ) {
		...
		G_Trace( &tr, ent->client->ps.origin, ent->r.mins, vMax, ent->client->ps.origin, ent->s.number, mask );
	} else {
		G_Trace( &tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, ent->s.pos.trBase, ent->s.number, mask );
	}
...
qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	*obstacle = NULL;
	...
	listedEntities = G_EntitiesInBox( totalMins, totalMaxs, entityList, MAX_GENTITIES, pusher->s.number );
...
void SP_func_pendulum(gentity_t *ent) {
	...
	freq = 0.5f * (float) M_1_PI * sqrtf( g_gravity.value / ( 3 * length ) );
