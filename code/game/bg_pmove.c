...
static qboolean PM_CheckJump( void )
{
						...
						{ //jump is already active (the anim has started)
							...
							{//not in the middle of a legsAnim
								...
								case BOTH_FORCEJUMPRIGHT1:
									...
									break;
								default:
									newAnim = ANIM_INVALID;
									break;
								}
								if ( newAnim != ANIM_INVALID )
								{
									int parts = SETANIM_BOTH;
									if ( pm->ps->weaponTime )
									{
	...
	{//okay, we just jumped and we're in an attack
		...
		{//not in an anim we shouldn't interrupt
			...
			int animLength = PM_AnimLength( ANIM(pm->ps->torsoAnim) );
...
================
*/
static entity_event_t PM_FootstepForSurface( void )
{
...
static animNumber_t PM_TryRoll( void )
{
	trace_t	trace;
	animNumber_t	anim = ANIM_INVALID;
	vec3_t fwd, right, traceto, mins, maxs, fwdAngles;
...
static void PM_CrashLand( void ) {
	...
	{ //saber handles its own anims
		...
		if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == ZOOM_DISRUPTOR)
		{
...
static void PM_Footsteps( void ) {
	...
	if ( pm->ps->pm_flags & PMF_DUCKED )
	{
		animNumber_t rolled = ANIM_INVALID;
...
void PM_BeginWeaponChange( int weapon ) {
	...
	if (pm->ps->zoomMode != ZOOM_NONE)
	{
		pm->ps->zoomMode = ZOOM_NONE;
...
//---------------------------------------
{
	...
	case WP_DISRUPTOR:
		...
		if (pm->ps->zoomMode != ZOOM_DISRUPTOR &&
...
static void PM_Weapon( void )
{
	...
	{ //nothing else should be allowed to happen during this time, including weapon fire
		animNumber_t desiredAnim;
		qboolean seperateOnTorso = qfalse;
		animNumber_t desiredOnTorso;
		...
		case HANDEXTEND_DODGE:
			desiredAnim = (animNumber_t)pm->ps->forceDodgeAnim;
			break;
	...
	if ( pm->cmd.buttons & BUTTON_USE_HOLDABLE ) {
		if ( ! ( pm->ps->pm_flags & PMF_USE_ITEM_HELD ) ) {
			...
			else
			{
				...
				pm->ps->pm_flags |= PMF_USE_ITEM_HELD;
				PM_AddEvent( (entity_event_t)(EV_USE_ITEM0 + bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag) );
	...
	if ((ANIM(pm->ps->torsoAnim) == TORSO_WEAPONREADY4 ||
		...
		(pm->ps->weapon != WP_DISRUPTOR || pm->ps->zoomMode != ZOOM_DISRUPTOR))
	{
		if (pm->ps->weapon == WP_EMPLACED_GUN)
		{
			PM_StartTorsoAnim( BOTH_GUNSIT1 );
	...
	if (pm->ps->weapon == WP_DISRUPTOR &&
		(pm->cmd.buttons & BUTTON_ALT_ATTACK) &&
		pm->ps->zoomMode == ZOOM_BINOCULARS)
...
//-------------------------------------------
{
	...
	if ( pm->ps->weapon == WP_DISRUPTOR && pm->ps->weaponstate == WEAPON_READY )
	{
			...
			pm->cmd.upmove <= 0 && !pm->cmd.forwardmove && !pm->cmd.rightmove*/)
		{
			...
			else if (pm->ps->zoomMode == ZOOM_DISRUPTOR && pm->ps->zoomLockTime < pm->cmd.serverTime)
		...
		*/
		...
		{
			if (pm->ps->zoomMode == ZOOM_DISRUPTOR && pm->ps->zoomLockTime < pm->cmd.serverTime)
			{ //check for == 1 so we can't turn binoculars off with disruptor alt fire
				pm->ps->zoomMode = ZOOM_NONE;
				pm->ps->zoomTime = pm->ps->commandTime;
				pm->ps->zoomLocked = qfalse;
	...
	if ( pm->ps->weapon == WP_DISRUPTOR)
	{
		if ( pm->cmd.buttons & BUTTON_ATTACK && pm->ps->zoomMode == ZOOM_DISRUPTOR && pm->ps->zoomLocked)
		{
			// converting the main fire to an alt-fire
			pm->cmd.buttons |= BUTTON_ALT_ATTACK;
			pm->ps->eFlags |= EF_ALT_FIRING;
		}
		else if ( pm->cmd.buttons & BUTTON_ALT_ATTACK && pm->ps->zoomMode == ZOOM_DISRUPTOR && pm->ps->zoomLocked)
		{
			pm->cmd.buttons &= ~BUTTON_ALT_ATTACK;
			pm->ps->eFlags &= ~EF_ALT_FIRING;
		}
...
void PmoveSingle (pmove_t *pmove) {
	...
	if ( abs( pm->cmd.forwardmove ) > 46 || abs( pm->cmd.rightmove ) > 46 ) {
